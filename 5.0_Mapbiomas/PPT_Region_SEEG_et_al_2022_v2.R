...
##############################
all_mun1 <- read_municipality(year=2020, code_muni = "all")
...
top100<- dataset_final1 %>%
  group_by(name_muni)%>%
  filter(name_region == "Norte")%>% 
  # selecionar o top 10
  top_n(10, VALOR)    

topOKKK <- dataset_finalN  %>%
...
W10 <- rbind(Wata1,Wata2,Wata3,Wata4,Wata5,Wata6,Wata7,Wata8,Wata9,Wata10)
  
  
  filter( == "1" |rank == "2" |rank == "3"| rank == "4"|rank == "5"|
...
ggplot() +
  ...
  geom_sf(data=d1, aes(fill=VALOR), size=.125, color=alpha("orange",0.05))+

  
W10$VALOR=as.numeric(levels(W10$VALOR))[df1000$VALOR]

p <- ggplot(W10, aes(rank, 
                         fill = VALOR, color = as.factor(`Nome_Município`)))+
  #scale__discrete()+
  ...
  scale_fill_distiller(palette = "Oranges", type="seq", name="Mt Co2eq",
                       limits = c(min(W10$VALOR, max(W10$VALOR))))+
